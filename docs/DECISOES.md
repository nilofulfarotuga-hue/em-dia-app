...
## D46 — Não se compra domínio: o Em Dia vive em subdomínios de boraguarda.com
- **O quê (decisão do Danilo, 7 de setembro de 2026, 00h00):** `emdia.boraguarda.com` (site), `app.emdia.boraguarda.com` (app web), `admin.emdia.boraguarda.com` (painel); remetente `emdia@boraguarda.com` (domínio já verificado na Resend); caixa das faturas em `faturas.boraguarda.com` se o Email Routing aceitar subdomínio, senão `emdia-<codigo>@boraguarda.com` por regra catch-all. Os `*.pages.dev` continuam a existir e redirecionam para o endereço novo. `em-dia.app` sai da lista de pendentes.
...
## D68 — O ícone da app é a proposta 1 (calendário com visto verde) até o Danilo escolher outro
- **O quê (2026-09-18):** `python tool/marca/desenhar.py icones --n 1` gerou `assets/branding/icon.png` e `icon_foreground.png`, `web/favicon.png` e `web/icons/Icon-{192,512}.png` + maskable; `dart run flutter_launcher_icons` gerou os mipmaps do Android (adaptativo, fundo branco). O `pubspec` deixa de gerar a web pelo launcher_icons (apagava o nome, a língua e a descrição do manifest).
- **Porquê:** a app estava no ar com o ícone azul de fábrica do Flutter na web, no Android e no favicon. Das 5 propostas desenhadas a 06/09, a 1 teve a nota mais alta do juiz (26 pontos; `docs/marca/pontuacoes.json`) e é a mais legível a 48 px. Trocar depois é um comando (`--n N` + launcher_icons).
- **Como se desfaz:** correr os dois comandos com outra proposta, ou com um ficheiro do Danilo em `assets/branding/icon.png`.

## D69 — A app na web guarda-se a si própria: service worker nosso, não o do Flutter
- **O quê:** `web/sw.js` (rede primeiro para a página, o `flutter_bootstrap.js` e o `main.dart.js`; cache primeiro para o CanvasKit, os tipos de letra e os assets; Supabase, Turnstile e o resto passam intactos) e `web/flutter_bootstrap.js` (modelo que chama o carregador do Flutter SEM `serviceWorkerSettings` e regista o `sw.js` depois do primeiro ecrã). Depois do primeiro ecrã a página manda ao worker a lista do que carregou e ele guarda-a (do HTTP cache, sem descarregar de novo). O CI carimba o commit em `__VERSAO__`: worker novo a cada deploy, cache antiga apagada.
- **Porquê:** o `flutter_service_worker.js` do Flutter 3.47 é um resto deprecado (flutter/flutter#156910) que se desregista a si próprio e recarrega a página — a app nunca abria sem rede (Bloco 0). Uma lista fixa de ficheiros a guardar envelhecia a cada versão do Flutter (nomes do CanvasKit com hash); guardar «o que a página carregou» não envelhece.
- **Como se desfaz:** apagar `web/sw.js` e `web/flutter_bootstrap.js`; o `flutter build web` volta ao bootstrap de fábrica. Um worker já instalado nos telemóveis morre sozinho quando `sw.js` passar a 404? Não — fica até ser substituído; por isso, para desligar a sério, deixa-se um `sw.js` que só faz `registration.unregister()`.

## D70 — Apagar conta a sério: simulação primeiro, motivo obrigatório, Edge Function com a service role
- **O quê (2026-09-18):** no painel, «Apagar conta» passa a ser definitivo: 1) `admin_apagar_conta_simular` mostra o que vai embora (linhas por tabela, ficheiros no Storage); 2) o admin escreve o motivo; 3) a Edge Function `admin-apagar-conta` apaga os ficheiros (`comprovativos/`, `faturas/`) pela API do Storage e a conta em `auth.users` (o resto cai por `ON DELETE CASCADE`; conversas com a IA e tickets ficam anónimos por `SET NULL`) e regista em `admin_audit_log` (`usuario_apagado`, com os números e o motivo). Travas: nunca um admin, nunca a própria conta, nunca sem motivo. A porta de QA (`x-cron-secret`) só simula.
- **Porquê:** «apagar/banir» era o pedido do Bloco 7 e o botão antigo só marcava «banido» e deixava o apagar «para o servidor depois» — que não existia. Apagar linhas de `storage.objects` por SQL deixava os ficheiros órfãos no disco; por isso é a API do Storage, que só a service role tem.
- **Como se desfaz:** o botão volta a chamar `banir(u, true)`; a função e a Edge Function ficam sem uso.

## D71 — Assinaturas e erros de leitura são secções do painel, lidas por RPC só-admin
